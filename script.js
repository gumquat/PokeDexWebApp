...
async function loadMorePokemon()
{
    ...
    let remainingPokemon = pokemonCount - currentLoadedCount;
    let pokemonToLoad = Math.min(remainingPokemon, pokemonPerLoad);
    let end = start + pokemonToLoad - 1;
    ...
    // Check if new Pokémon were actually loaded
    if (currentLoadedCount >= start) {
        for (let i = start; i <= currentLoadedCount; i++) {
...
async function getPokemonBulk(start, end)
{
    ...
    try {
        ...
        // We don't revert currentLoadedCount here, as it wasn't updated in case of an error
    }
}
